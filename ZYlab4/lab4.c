...
#define  total_instruction  320     //Length of instruction stream
#define  total_vp  32               //Length of virtual page
#define  clear_period  50           //Period of clear

typedef struct   pl_type                 //Structure of page
{
	int pn;
	int pfn;
	int counter;
...
struct pl_type pl[total_vp];               //Array of page structure

struct pfc_struct               //Page control structure
{                 
	int pn;
	int pfn;
...
int initialize(int total_pf)              //Initialize
	                        //Number of Memory page
{
	...
	for(i=0;i<total_pf-1;i++)
	{
		pfc[i].next=&pfc[i+1];
		pfc[i].pfn=i;
	}   //Link pfc[i-1] and pfc[i]
	pfc[total_pf-1].next=NULL;
	pfc[total_pf-1].pfn=total_pf-1;
	freepf_head=&pfc[0];         //Head pointer of empty page queue

	return 0;
}


int FIFO(int total_pf)              //First-in-first-out
{
	int i,j;
	pfc_type *p;
	initialize(total_pf);         //Initialize
	busypf_head=busypf_tail=NULL; //忙页面队列头，队列尾链接
	for(i=0;i<total_instruction;i++)
	{
		if(pl[page[i]].pfn==INVALID)   //Page is invalid
		{
			diseffect+=1;                  //Cnt of invalid pages
